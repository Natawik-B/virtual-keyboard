...
const Keyboard = {
  ...
  init() {
    ...
    const input = document.querySelector('.keyboard__table-text');
    document.querySelectorAll('.keyboard__table-text').forEach((element) => {
      ...
      element.addEventListener('keypress', (key) => {
        ...
        this.properties.start += 1;
        this.properties.end += 1;
        // console.log(key.key);
      });
      element.addEventListener('keydown', (key) => { // выделение текста при влево/вправо
        if (key.which === 37) {
          this.properties.start -= 1;
          this.properties.end -= 1;
        ...
        if (key.which === 39) {
          ...
          if (this.properties.start > this.properties.value.length) {
            this.properties.start = this.properties.value.length;
          } else if (this.properties.end > this.properties.value.length) {
            this.properties.end = this.properties.value.length;
          }
        }
      });
      input.focus();
      ...
      switch (key) {
        case 'backspace':
          ...
          keyElement.addEventListener('click', () => {
            ...
            this.properties.value = this.properties.value.substring(0, index - 1)
            + this.properties.value.substring(index);
        ...
        case 'shift':
          ...
          keyElement.addEventListener('click', () => {
            ...
            input.focus();
            for (let i = 0; i < keyLayout.length; i += 1) { // смена вида символов
            ...
            document.querySelector('.right').addEventListener('click', () => {
              if (!this.properties.shift) {
                ...
                if (this.properties.start <= this.properties.end && this.properties.direction === 'forward') {
                  this.properties.end += 1;
                  if (this.properties.end > this.properties.value.length) {
                    this.properties.end = this.properties.value.length;
                  }
                } else this.properties.start += 1;
